...
const getFieldsArray = (obj)=>{
    ...
    return fields.map((field)=>{
        ...
        else if(field['argType'] === 'checkbox')
        {
            ...
            let title = argList[0];
            let value = argList[1];
            let arg_name = field['argName'].replace(/ /g,'_');
...
class ReadConfig {
    ...
    {
        if(this.fields === null)
            this.fields = getFields(this.conf);
    ...
    getFieldsArray()
    {
        if(this.fieldsArray === null)
            this.fieldsArray = getFieldsArray(this.conf);
